...
ut=-(9.8/sw.f(lat.data))*np.gradient(adt,axis=0)/dy
vt=(9.8/sw.f(lat.data))*np.gradient(adt,axis=1)/dx
del adt
...
up=-(9.8/sw.f(lat.data))*np.gradient(sla,axis=0)/dy
vp=(9.8/sw.f(lat.data))*np.gradient(sla,axis=1)/dx
del sla
...
mke = 0.5*(np.nanmean(ut,axis=0)**2 + np.nanmean(vt,axis=0)**2)
...
p=plt.figure(figsize=(7.49, 6.24))
...
cc=C.contourf(lonx,latx,mke,cmap='RdYlBu_r',levels=np.linspace(15,130,20))
...
cbar=plt.colorbar(cc,ticks=np.arange(10,130,20),format='%i',aspect=50,pad=0.02)
cbar.set_label(r'mEKE/MKE [%]')
...
plt.tight_layout()
plt.savefig('/home/igor/Documents/AntSimi/figures/clim/EKE_MKE.png')
...
for ix in range(4):
    ...
    smke=np.nanmean(mke[cond,:,:],axis=0)
    smeke=np.nanmean(meke[cond,:,:],axis=0)
    ...
    cc=C.contourf(lonx,latx,(smeke/smke)*100,cmap='RdYlBu_r',levels=np.linspace(10,130,20),extend='max')
    # Q=C.quiver(lon,lat,und,vnd,scale=10)
    # plt.quiverkey(Q,0.11,0.9,0.2,r'0.2 m s $^{-1}$')
...
# plt.clabel(ctt,fmt='%i',fontsize=10)
    plt.tight_layout()
    plt.savefig('/home/igor/Documents/AntSimi/figures/clim/EKE_MKE_{0}.png'.format(seastr[ix]))
    # plt.savefig('/home/igor/Documents/AntSimi/figures/clim/adt_clim.png')
    plt.close()
...
'MKE'
kratio=mke
# kratio=((np.nanmean(meke,axis=0))/(np.nanmean(mke,axis=0)))*100

kratio[maskx!=1]=np.nan;
...
p=plt.figure(figsize=(6.51, 6.24))
lbs=[True, False,False,True]
# C = Basemap(llcrnrlon=-49.5,llcrnrlat=-28,urcrnrlat=-23,urcrnrlon=-43.5,resolution='h',projection='merc')
C = Basemap(llcrnrlon=lnmn+2,llcrnrlat=ltmn,urcrnrlat=ltmx,urcrnrlon=lnmx,resolution='h',projection='cyl')
...
cc=C.contourf(lonx,latx,kratio,cmap='RdYlBu_r',levels=np.linspace(0,0.06,20),extend='max')
# cc=C.contourf(lonx,latx,kratio,cmap='RdYlBu_r',levels=np.linspace(15,130,20))

# cc=C.pcolormesh(lonx,latx,kratio,cmap='RdYlBu_r',vmin=15,vmax=130)
...
cbar=plt.colorbar(cc,ticks=np.arange(0,0.07,0.01),format='%.3f',aspect=50,pad=0.02)
...
plt.tight_layout()
plt.savefig('/home/igor/Documents/AntSimi/figures/clim/MKE_mdt.png')
# plt.savefig('/home/igor/Documents/AntSimi/figures/clim/adt_clim.png')
plt.close()
...
cc=C.contourf(lonx,latx,kratio,cmap='RdYlBu_r',levels=np.linspace(0,0.04,20))
# cc=C.contourf(lonx,latx,kratio,cmap='RdYlBu_r',levels=np.linspace(15,130,20))
...
cbar=plt.colorbar(cc,ticks=np.arange(0,0.05,0.01),format='%.3f',aspect=50,pad=0.02)
...
plt.tight_layout()
plt.savefig('/home/igor/Documents/AntSimi/figures/clim/MEKE.png')
# plt.savefig('/home/igor/Documents/AntSimi/figures/clim/adt_clim.png')
plt.close()
...
print('Mean EKE/MKE = {0}'.format(np.nanmean(kratio)))
...
for ix in range(4):
    ...
    cc=C.contourf(lonx,latx,kratio,cmap='RdYlBu_r',levels=np.linspace(10,130,20),extend='max')
    # Q=C.quiver(lon,lat,und,vnd,scale=10)
    # plt.quiverkey(Q,0.11,0.9,0.2,r'0.2 m s $^{-1}$')
    ...
    C.contourf(lonb,latb,bat,levels=[-150,np.inf],colors='dimgray',alpha=1,latlon=True,zorder=14)
    C.contour(lonb,latb,-bat,levels=[150],colors='k',alpha=1,linewidth=0.5,latlon=True,zorder=15)
    # C.contour(lonb,latb,-bat,levels=[-np.inf,150],colors='k',alpha=1,latlon=True,linewidhts=0.5,zoder=16)
    # ctt=C.contour(lonb,latb,-bat,levels=[1e3,2e3,3e3],colors='dimgray',alpha=0.8,latlon=True,linewidths=0.5)
...
# plt.clabel(ctt,fmt='%i',fontsize=10)
    plt.tight_layout()
    plt.savefig('/home/igor/Documents/AntSimi/figures/clim/masked_EKE_MKE_{0}.png'.format(seastr[ix]))
    # plt.savefig('/home/igor/Documents/AntSimi/figures/clim/adt_clim.png')
    plt.close()

    print('Mean EKE/MKE {0} = {1}'.format(seastr[ix],np.nanmean(kratio)))
...
kratio=((np.nanmean(mke,axis=0)))*100
# kratio=((np.nanmean(meke,axis=0))/(np.nanmean(mke,axis=0)))*100
...
plt.tight_layout()
plt.savefig('/home/igor/Documents/AntSimi/figures/clim/masked_EKE_MKE.png')
# plt.savefig('/home/igor/Documents/AntSimi/figures/clim/adt_clim.png')
plt.close()
