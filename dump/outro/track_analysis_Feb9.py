...
'For percentage in specific regions'
def perc_region(lon,lat,map,ltmin,ltmax,lnmin,lnmax):
    finder=np.argwhere( ((lat<=ltmax)&(lat>=ltmin)) & ((lon<=lnmax)&(lon>=lnmin)) ).T[0]
    result= np.nansum(map[finder])/np.nansum(map)
    print('{0} = {1:0.2f} %'.format(eddyn,result))
    return result
...
for i in range(4):
    ...
    eloc=[[-16,-14,-39.5,-37.5,acmap,'IE'],
                [-18,-16,-39,-37,acmap,'RCE'],
                [-20,-18,-38,-36,acmap,'AE'],
                [-21.4,-19.25,-40.250,-37.750,ccmap,'VE'],
                [-22.5,-20.5,-40.725,-38.725,ccmap,'SVE'],
                [-24,-22,-41,-39,ccmap,'CSTE'],
                [-25,-23,-43,-41,ccmap,'CFE']]
    ...
    plt.pcolormesh(lonf,latf,((acmap+ ccmap)/(np.sum(acmap) + np.sum(ccmap)))*100,cmap=cm.cm.tempo,vmin=0,vmax=1);
    plt.pcolormesh(lonf,latf,((acmap+ ccmap)/(anumber + cnumber))*100,cmap=cm.cm.tempo,vmin=0,vmax=1);
    ...
    plt.pcolormesh(lonf,latf,((ccmap)/(cnumber))*100,cmap='Blues',vmin=0.2,vmax=1);
    ...
    plt.pcolormesh(lonf,latf,((acmap)/(anumber))*100,cmap='Reds',vmin=0.2,vmax=1);
    plt.text(lonf.min()+2,latf.max()-2,'{0}'.format(mosea[i]))
    cbar=plt.colorbar()
    cbar.set_label('Formation rate [%]',fontweight='bold') 
    ...
    print('############ {0} ############'.format(mosea[i]))
    for aia in range(len(eloc)):
        ltmin,ltmax,lnmin,lnmax,emap,eddyn=eloc[aia]
        _=perc_region(lonf,latf,emap,ltmin,ltmax,lnmin,lnmax)
...
# C.contourf(lonf,latf,((acmap+ ccmap)/np.nanmax(acmap + ccmap))*100,cmap='YlGnBu_r',latlon=True,levels=np.linspace(0,60,35))
# plt.pcolormesh(lonf,latf,((acmap+ ccmap)/np.nanmax(acmap + ccmap))*100,cmap=cm.cm.tempo,vmin=0,vmax=80);
plt.pcolormesh(lonf,latf,((acmap+ ccmap)/(anumber + cnumber))*100,cmap=cm.cm.tempo,vmin=0,vmax=3);
...
C.contourf(lonf,latf,((acmap - ccmap)/np.sum(acmap-ccmap))*100,cmap='seismic',latlon=True,levels=np.linspace(-50,50,35))
...
plt.pcolormesh(lonf,latf,((ccmap)/cnumber)*100,cmap='Blues',vmin=0,vmax=5);
...
C.contour(xxb,yyb,-zb,colors='k',alpha=1,levels=[0,100],linewidths=0.5,latlon=True)
# C.contourf(lonf,latf,(acmap - ccmap)/np.nanmax(abs(acmap - ccmap))*100,cmap='seismic',latlon=True,levels=np.linspace(-100,100,35))
plt.pcolormesh(lonf,latf,((acmap)/anumber)*100,cmap='Reds',vmin=0,vmax=5);
...
plt.pcolormesh(lonf,latf,(nlmap)/(adummy+cdummy),cmap=cm.cm.tempo,vmin=0,vmax=6);
