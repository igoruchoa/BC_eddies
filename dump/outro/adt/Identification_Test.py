...
'Station'
path='/home/igor/Documents/AntSimi_adt/data/'
...
for ix in range(len(files)):
    ...
    'Filter'
    h.high_filter('adt', 10, 5)
    # h.bessel_high_filter('sla', 500, order=3)

    ac, cc = h.eddy_identification('adt', 'ugos', 'vgos', date=dt,step= 0.0025)
    # ac, cc = h.eddy_identification('sla', 'ugosa', 'vgosa', date=ddt,step= 0.0025)



    with Dataset('/home/igor/Documents/AntSimi_adt/tmp/Anticyclonic_{0}.nc'.format(dt.strftime('%Y%m%d') ), 'w') as h:
        ac.to_netcdf(h)
    with Dataset('/home/igor/Documents/AntSimi_adt/tmp/Cyclonic_{0}.nc'.format(dt.strftime('%Y%m%d') ), 'w') as h:
        cc.to_netcdf(h)
